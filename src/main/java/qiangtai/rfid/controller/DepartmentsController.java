...
public class DepartmentsController {
    ...
    public Result<Page<DepartmentsResultVO>> pageDepartments(@ParameterObject DepartmentQuery departmentQuery) {
        return Result.success(departmentsService.pageDepartments(departmentQuery));
    }
    @GetMapping("/listDepartments")
    @Operation(summary = "部门少,列表查看部门")
    public Result<List<DepartmentsResultVO>> listDepartments(@ParameterObject DepartmentQuery departmentQuery) {
        return Result.success(departmentsService.listDepartments(departmentQuery));
    }
